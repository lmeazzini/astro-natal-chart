...
def upgrade() -> None:
    ...
    op.add_column(
        "birth_charts",
        sa.Column("pdf_generating", sa.Boolean(), server_default=sa.false(), nullable=False),
    )
    ...
    op.create_index(
        op.f("ix_birth_charts_pdf_generating"), "birth_charts", ["pdf_generating"], unique=False
    )
    op.create_index(
        op.f("ix_birth_charts_pdf_task_id"), "birth_charts", ["pdf_task_id"], unique=False
    )
